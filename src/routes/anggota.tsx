...
export const Route = createFileRoute("/anggota")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Kenali pengurus, anggota, alumni, dan mitra perusahaan di ekosistem HMTI.",
      },
          ...
          <div className="flex gap-2">
            {TABS.map((t) => (
              <button
                key={t}
                onClick={() => setTab(t)}
                className={`px-4 h-10 rounded-md font-heading text-xs uppercase tracking-widest border transition-colors ${tab === t ? "bg-red text-bone border-red" : "border-line text-bone-muted hover:text-red hover:border-red"}`}
              >
                {t}
              </button>
            ))}
        ...
        <div className="mx-auto max-w-7xl px-5 lg:px-8">
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
            {list.map((m) => (
              <article
                key={m.id}
                className="group rounded-xl border border-line bg-surface p-6 card-hover flex flex-col items-center text-center"
              >
                <span className="px-2.5 py-1 rounded bg-red/15 text-red font-mono text-[10px] uppercase tracking-widest mb-3">
                  {m.type}
                </span>
                <img
                  src={m.avatar}
                  alt={m.name}
                  className="w-24 h-24 rounded-full ring-2 ring-red/50 object-cover"
                />
                ...
                <div className="mt-4 flex gap-2">
                  {[Github, Linkedin, Mail].map((Icon, i) => (
                    <a
                      key={i}
                      href="#"
                      className="w-8 h-8 grid place-items-center rounded border border-line text-bone-muted hover:text-red hover:border-red transition-colors"
                    >
          ...
          <div className="mt-20">
            ...
            <p className="text-bone-muted mb-8">
              Perusahaan tempat alumni HMTI berkarya dan bermitra.
            </p>
